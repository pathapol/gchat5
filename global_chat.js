$(function() {
  ...
  function sendMessage() {
    message = new Date().toISOString().replace(/T/, ' ').replace(/\..+/, '')+ " "+$messageInput.val().trim();
    messagess = $messageInput.val().trim();
    if (message) {
      //t2 = message.substring(0,19);
      //var hourss = current_hour;
      time = new Date().toISOString().replace(/T/, ' ').replace(/\..+/, '');
      $messageInput.val('');
      createChatMessage(message, username, time, messagess);//time);
      socket.emit('new message', message);

      //ocket.emit('new time', time);
    }
  }
  ...
  function createChatMessage(message, user, hour, messagess) {
    len = message.length;
    var type = '';
    if (user === username) {
      type = 'chat-user';
    ...
    msg = message.substring(19,message.length-1);
    //msg = message + message.length;
    $li.find('p').text(msg);//messagess);//message);
    $li.find('b').text(user);
    //var current_hours = current_hour;
    hour = message.substring(0,19) ;
    $li.find('loo').text(hour);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  socket.on('new message', function(data){
    //var hourss = current_hour;
    hour = new Date().toISOString().replace(/T/, ' ').replace(/\..+/, '');
    createChatMessage(data.message, data.username, hour, data.message);
  });
